...
def extract_timestamp_from_model_path(model_path):
    ...
    # Modify regex to match timestamp with _A, _B, _C suffix
    timestamp_match = re.search(r'(\d{4}-\d{2}-\d{2}_\d{2}-\d{2}:\d{2})(?:_[ABC])?', filename)
    print(f"timestamp_match: {timestamp_match}")
    if timestamp_match:
        timestamp = timestamp_match.group(1)
        # Extract regularization scheme from filename
        scheme_match = re.search(r'_([ABC])_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}:\d{2}_[ABC]\.pth$', filename)
        if scheme_match:
            scheme = scheme_match.group(1)
            return f"{timestamp}_{scheme}"
        return timestamp
    else:
...
def evaluate_adaptive_ddpm(model_path):
    ...
    # Modification: Use correct initialization parameters - AdaptiveDDPM does not accept D parameter
