...
class AdaptiveDDPM(nn.Module):
    ...
    def get_L(self):
        """Generate lower triangular Cholesky factor matrix, ensure numerical stability"""
...
def train_adaptive_ddpm(model, dataloader, num_epochs, device, config):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if current_val_loss < best_val_loss:
            ...
            # Save model state dict as CPU tensors to avoid potential CUDA memory issues
...
def inference_with_low_rank_noise(model, N_test, scaler, config, U_k, Lambda_k_sqrt_diag, k, device):
    ...
    # Note: This function doesn't print ">>> Step 5" title because it's called within timing loop
    model.eval()
    betas = config['betas']
    alphas = config['alphas']
    alpha_bars = config['alpha_bars']
    ...
    with torch.no_grad():
        # Initial noise (can still use model.get_noise() or low-rank generation, theoretically x_T doesn't matter much, but use low-rank for consistency)
        xi_k_init = torch.randn(N_test, k, device=device)
        x_t = (generator_k @ xi_k_init.T).T # (T x k) @ (k x B) -> T x B -> B x T

        # for t in tqdm(reversed(range(T_local)), desc=f"Sampling (k={k})", leave=False): # leave=False to avoid leaving a progress bar for each k
        ...
        samples = x_t.cpu().numpy()
        # samples = scaler.inverse_transform(x_t.cpu().numpy()) # If inverse standardization needed
    return samples
...
def measure_noise_generation_speed(model_path, k_values, device, batch_size=128, num_repeats=10_000):
    ...
    for i, k in enumerate(sorted(k_values)): # Best to calculate cumulative values in order of k
        if k > T:
...
def measure_inference_time_for_k(k_value, model, N_test, scaler, config, device,
    ...
    # print(f"   Testing inference time for k={k_value}...") # Don't print too much info inside

    model.eval() # Ensure model is in evaluation mode
    T_local = config['T']

    # --- Depending on k_value, select inference function and prepare parameters ---
    if k_value == 'full':
    ...
    # --- Warm-up ---
    # print(f"     Performing warm-up (k={k_value})...")
    for _ in range(num_warmup):
        _ = inference_func(*params)
    # print(f"     Warm-up completed (k={k_value}).")
    ...
    torch.cuda.synchronize()
    start_event.record()
    # Use tqdm to display overall progress instead of showing progress for each inference
    for _ in range(num_repeats):
         _ = inference_func(*params)
    end_event.record()
    torch.cuda.synchronize()
...
def analyze_model_matrices(model, save_path="matrix_debug"):
    ...
    print(f"Is Sigma Symmetric: {Sigma_stats['is_symmetric']}")
    print(f"Is Sigma Positive Definite: {Sigma_stats['is_positive_definite']}")
...
def run_adaptive_ddpm():
    ...
    if not os.path.exists(model_save_path) and not os.path.exists(last_model_save_path):
        ...
        try:
            ...
            print(f"Failed to load model {model_path_to_load}: {e}")
            ...
            # Try to load model parameters directly
            print("Try directly checking model parameters...")
            ...
            print("\n>>> Model loading failed, use initialized model for analysis...")
            analyze_model_matrices(model, save_path="matrix_debug_fallback")
            
            # Re-train a simplified backup model as a fallback
    ...
    # --- Precompute feature decomposition (if model loaded successfully) ---
    eigenvalues, eigenvectors, sqrt_eigenvalues, total_variance = None, None, None, None
    if model_loaded:
        print("Precompute covariance matrix feature decomposition...")
        with torch.no_grad():
            L = model.get_L().detach()
    ...
    if model_loaded:
        ...
        # ... (Print noise speed table code remains unchanged) ...
    else:
        print("Cannot perform noise speed test because model loading or feature decomposition failed.")

    # Step 7: Test full inference time for different k values
    if model_loaded:
        ...
        print(f"\n>>> Start testing full inference time for different k values (batch size: {inference_batch_size})")
        ...
        # Print results, sorted by k ('full' comes first)
        ...
        print("----------------------------------------------------------")
         ...
         print("Cannot perform inference time test because model loading or feature decomposition failed.")
