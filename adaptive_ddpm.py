...
# DIFFWAVE_LR = 1e-6
# DIFFWAVE_LR = 5e-5
# logger.info(f"DIFFWAVE_LR: {DIFFWAVE_LR}") # Converted print to logger.info - Move this logging later

# BATCH_SIZE = 2048
# logger.info(f"BATCH_SIZE: {BATCH_SIZE}") # Converted print to logger.info - Move this logging later
# ---------------------------------- #A20

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
# logger.info(f"Using device: {device}") # Converted print to logger.info - Move this logging later
...
class AdaptiveDDPM(nn.Module):
    ...
    def forward(self, x, t):
        ...
        # "Block" flatten: first all 'a' values, then all 'b' values
...
def evaluate_metrics(real_data, gen_data):
    ...
    2. MSE (mean squared error between correlation matrix elements)
    3. MAE (mean absolute error between correlation matrix elements)
...
def train_adaptive_ddpm(model, dataloader, num_epochs, device, config):
    ...
    lambda_1 = float(os.environ.get("HP_LAMBDA_1", "1e-2"))  # Frobenius regularization
    # lambda_1 = float(os.environ.get("HP_LAMBDA_1", "0"))  # Frobenius regularization
    lambda_2 = float(os.environ.get("HP_LAMBDA_2", "0"))  # Trace regularization
    # lambda_2 = float(os.environ.get("HP_LAMBDA_2", "1e-2"))  # Trace regularization
    # lambda_2 = float(os.environ.get("HP_LAMBDA_2", "1e-3"))  # Trace regularization
    logger.info(f"Lambda 1 (Frobenius): {lambda_1}, Lambda 2 (Trace): {lambda_2}")
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        logger.info(f"Epoch {epoch+1}/{num_epochs} - TB Loss/train: {train_loss/len(train_loader):.4f}")
        writer.add_scalar('Loss/train_recon', train_recon/len(train_loader), epoch)
        logger.info(f"Epoch {epoch+1}/{num_epochs} - TB Loss/train_recon: {train_recon/len(train_loader):.4f}")
        writer.add_scalar('Loss/train_reg', train_reg/len(train_loader), epoch)
        logger.info(f"Epoch {epoch+1}/{num_epochs} - TB Loss/train_reg: {train_reg/len(train_loader):.4f}")
        writer.add_scalar('Loss/val', avg_val_loss, epoch)
        logger.info(f"Epoch {epoch+1}/{num_epochs} - TB Loss/val: {avg_val_loss:.4f}")
        writer.add_scalar('Learning_Rate', current_lr, epoch)
        logger.info(f"Epoch {epoch+1}/{num_epochs} - TB Learning_Rate: {current_lr:.2e}")
        ...
        #         f"LR: {current_lr:.2e}"
        #         f" | FN: {fro_diff:.4f}, MSE: {mse_val:.4f}, MAE: {mae_val:.4f}, Corr: {corr_val:.4f}, WD: {wd_val:.4f}")
        logger.info(f"Epoch {epoch+1}/{num_epochs} | Train Loss: {train_loss/len(train_loader):.4f} (Recon: {train_recon/len(train_loader):.4f}, Reg: {train_reg/len(train_loader):.4f}) | Val Loss: {avg_val_loss:.4f} (Recon: {val_recon/len(val_loader):.4f}, Reg: {val_reg/len(val_loader):.4f}) | LR: {current_lr:.2e} | FN: {fro_diff:.4f}, MSE: {mse_val:.4f}, MAE: {mae_val:.4f}, Corr: {corr_val:.4f}, WD: {wd_val:.4f}")
...
def inference(model, N_test, scalers, config):
    ...
    global logger # Access the global logger
    logger.info(">>> Step 5: Sampling generation")
    model.eval()
    betas = config['betas']
    alphas = config['alphas']
    alpha_bars = config['alpha_bars']
...
def run_adaptive_ddpm():
    ...
    global logger # Declare that we are modifying the global logger variable
    logger = logging.getLogger(__name__) # Initialize the global logger
    ...
    # Log initial hyperparameters and device after logger is initialized
    logger.info(f"DIFFWAVE_LR: {DIFFWAVE_LR}") # Moved logging here
    logger.info(f"BATCH_SIZE: {BATCH_SIZE}") # Moved logging here
    logger.info(f"Using device: {device}") # Moved logging here
    ...
    config = {
        ...
        'T': T,
        'device': device,
        'time_stamp': get_timestamp(), # Still keep a timestamp for internal filenames
    ...
    # Log initial run parameters after logger is initialized
    ...
    # After early stopping, we should load the best saved model weights for inference
